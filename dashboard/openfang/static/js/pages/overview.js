...
function overviewPage() {
  return {
    ...
    usageSummary: {},
    recentAudit: [],
    /** Bumps on i18n:language-changed so Alpine re-evaluates audit rows. */
    i18nTick: 0,
    channels: [],
    providers: [],
    mcpServers: [],
    ...
    timeAgo(timestamp) {
      if (window.i18n && window.i18n.timeAgoLocalized) return window.i18n.timeAgoLocalized(timestamp);
      if (!timestamp) return '';
      var now = Date.now();
