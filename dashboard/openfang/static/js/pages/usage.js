...
function analyticsPage() {
  return {
    ...
    async loadUsage() {
      ...
      try {
        ...
        this.loadError = e.message || (window.i18n && window.i18n.isInitialized()
          ? window.i18n.t('analytics.load_error')
    ...
    barChartData() {
      ...
      var lang = window.i18n && window.i18n.isInitialized() && window.i18n.getLanguage
        ? window.i18n.getLanguage()
        : 'en';
      var locale = lang === 'zh-CN' ? 'zh-CN' : lang === 'ru' ? 'ru' : 'en';
      var result = [];
      for (var i = 0; i < days.length; i++) {
        var d = new Date(days[i].date + 'T12:00:00');
        var dayName = d.toLocaleDateString(locale, { weekday: 'short' });
    ...
    barChartSvg() {
      ...
      for (var i = 0; i < bars.length; i++) {
        ...
        var costLabelY = y - 4;
        var callsLabel = window.i18n && window.i18n.isInitialized()
          ? window.i18n.t('analytics.n_calls', { n: bar.calls })
          : bar.calls + ' calls';
        var title = bar.date + ': ' + this.formatCost(bar.cost) + ' (' + callsLabel + ')';
