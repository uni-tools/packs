...
function workflowBuilder() {
  return {
    ...
    nodePaletteLabel: function(nt) {
      if (!nt || !nt.type) return '';
      var key = 'workflows.node.' + nt.type;
      if (typeof window.t === 'function') {
        var tr = window.t(key);
        if (tr !== key) return tr;
      }
      return nt.label || nt.type;
    },

    stepsConnectionsSummary: function() {
      var steps = this.nodes.filter(function(n) { return n.type !== 'start' && n.type !== 'end'; }).length;
      var conns = this.connections.length;
      if (typeof window.t === 'function') {
        return window.t('workflows.steps_connections_fmt', { steps: steps, conns: conns });
      }
      return steps + ' steps, ' + conns + ' connections';
    },

    defaultNodeLabel: function(type) {
      var def = null;
      for (var i = 0; i < this.nodeTypes.length; i++) {
        if (this.nodeTypes[i].type === type) { def = this.nodeTypes[i]; break; }
      }
      var fallback = def ? def.label : type;
      var key = 'workflows.node.' + type;
      if (typeof window.t === 'function') {
        var tr = window.t(key);
        if (tr !== key) return tr;
      }
    ...
    duplicateNode: function(node) {
      ...
      if (newNode) {
        ...
        var suf = typeof window.t === 'function' ? window.t('workflows.duplicate_suffix') : ' copy';
        newNode.label = node.label + suf;
      }
    },
    ...
    async saveWorkflow() {
      ...
      try {
        ...
        OpenFangToast.success(typeof window.t === 'function' ? window.t('workflows.toast_saved') : 'Workflow saved!');
        this.showSaveModal = false;
      } catch(e) {
        OpenFangToast.error((typeof window.t === 'function' ? window.t('workflows.toast_save_failed') : 'Failed to save') + ': ' + e.message);
      }
    },
