...
function skillsPage() {
  return {
    ...
    runtimeBadge: function(rt) {
      ...
      var tp = typeof window.t === 'function' ? window.t('skills.runtime_prompt') : 'PROMPT';
      ...
      if (r === 'prompt_only' || r === 'prompt' || r === 'promptonly') return { text: tp, cls: 'runtime-badge-prompt' };
    ...
    sourceBadge: function(source) {
      ...
      if (!source) return { text: t('skills.source_local'), cls: 'badge-dim' };
      switch (source.type) {
        case 'clawhub': return { text: t('skills.source_clawhub'), cls: 'badge-info' };
        case 'openclaw': return { text: t('skills.source_openclaw'), cls: 'badge-info' };
        case 'bundled': return { text: t('skills.source_builtin'), cls: 'badge-success' };
        default: return { text: t('skills.source_local'), cls: 'badge-dim' };
      }
    },

    toolCountLabel: function(n) {
      var t = typeof window.t === 'function' ? window.t : function(k, p) { return k; };
      return t('skills.tool_count', { n: n || 0 });
    },
    ...
    async loadSkills() {
      ...
      try {
        ...
        this.skills = [];
        this.loadError = e.message || (typeof window.t === 'function' ? window.t('skills.load_error') : 'Could not load skills.');
      }
      this.loading = false;
    },
