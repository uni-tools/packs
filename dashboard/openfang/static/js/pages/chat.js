...
function chatPage() {
  ...
  return {
    ...
    tipTimer: null,
    /** Incremented on i18n:language-changed so tip text re-renders. */
    i18nTick: 0,
    get currentTip() {
    ...
    get modelSwitcherLabel() {
      if (this.modelDisplayName) return this.modelDisplayName;
      if (window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()) {
        return window.i18n.t('chat.model_label_fallback');
      }
      return 'Model';
    },

    get tokenFooterLabel() {
      if (this.tokenCount <= 0) return '';
      if (window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()) {
        return window.i18n.t('chat.footer_tokens', { n: this.tokenCount });
      }
      return '~' + this.tokenCount + ' tokens';
    },

    get filesFooterLabel() {
      if (!this.attachments.length) return '';
      if (window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()) {
        return window.i18n.t('chat.footer_files', { n: this.attachments.length });
      }
      return this.attachments.length + ' file(s)';
    },

    get combinedFooterStats() {
      if (this.tokenCount > 0) return this.tokenFooterLabel;
      if (this.attachments.length) return this.filesFooterLabel;
      return '';
    },

    get queueFooterBadge() {
      if (this.messageQueue.length <= 0) return '';
      if (window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()) {
        return window.i18n.t('chat.queue_count', { n: this.messageQueue.length });
      }
      return this.messageQueue.length + ' queued';
    },

    get queueHeaderBadge() {
      if (this.messageQueue.length <= 0) return '';
      if (window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()) {
        return window.i18n.t('chat.header_queued', { n: this.messageQueue.length });
      }
      return '+' + this.messageQueue.length + ' queued';
    },
    ...
    toolStatusLabel(tool) {
      if (!tool) return '';
      if (tool.running) {
        return window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()
          ? window.i18n.t('chat.tool_running') : 'running...';
      }
      if (tool.is_error) {
        return window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()
          ? window.i18n.t('chat.tool_error') : 'error';
      }
      if (tool.result && tool.result.length > 500) {
        var kb = Math.round(tool.result.length / 1024);
        return window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()
          ? window.i18n.t('chat.tool_kb', { n: kb }) : (kb + 'KB');
      }
      return window.i18n && window.i18n.isInitialized && window.i18n.isInitialized()
        ? window.i18n.t('chat.tool_done') : 'done';
    },

    init() {
      var self = this;

      if (!self._i18nTipListener) {
        self._i18nTipListener = function() {
          self._tipsInitialized = false;
          self.tips = [];
          self.i18nTick++;
        };
        window.addEventListener('i18n:language-changed', self._i18nTipListener);
      }
    ...
    initSlashCommands: function() {
      ...
      this.slashCommands = [
        ...
        { cmd: '/tool', desc: 'Invoke UTCP tool directly: /tool name {"k":"v"}' }
      ];
    ...
    executeSlashCommand(cmd, cmdArgs) {
      ...
      switch (cmd) {
        ...
        case '/a2a':
          ...
          break;
        case '/tool':
          self.executeUtcpToolCommand(cmdArgs);
          break;
      }
    },

    async executeUtcpToolCommand(rawInput) {
      var raw = (rawInput || '').trim();
      if (!raw) {
        this.messages.push({
          id: ++msgId,
          role: 'system',
          text: 'Usage: /tool <tool_name> {"arg":"value"}',
          meta: '',
          tools: []
        });
        this.scrollToBottom();
      ...
      var toolName = firstSpace >= 0 ? raw.slice(0, firstSpace).trim() : raw;
      var argsText = firstSpace >= 0 ? raw.slice(firstSpace + 1).trim() : '';
      var argsObj = {};
      if (argsText) {
        try {
          ...
          this.messages.push({
            id: ++msgId,
            role: 'system',
            text: 'Invalid tool args JSON. Example: /tool web_search {"query":"openfang"}',
            meta: '',
            tools: []
          });
          this.scrollToBottom();
      ...
      this.messages.push({
        id: ++msgId,
        role: 'user',
        text: '/tool ' + toolName + (argsText ? ' ' + argsText : ''),
        meta: '',
        tools: [],
        ts: Date.now()
      });
      ...
      try {
        var res = await OpenFangAPI.post('/api/utcp/tools/invoke', {
          tool_name: toolName,
          args: argsObj,
          confirm: true
        });
        if (res && res.ok) {
          this.messages.push({
            id: ++msgId,
            role: 'agent',
            text: 'UTCP tool `' + toolName + '` result:\n\n```json\n' + JSON.stringify(res.data, null, 2) + '\n```',
            meta: 'utcp',
            tools: [],
            ts: Date.now()
          });
        ...
        this.messages.push({
          id: ++msgId,
          role: 'system',
          text: 'UTCP invoke request error: ' + (e2.message || 'unknown error'),
          meta: 'utcp',
          tools: [],
          ts: Date.now()
        });
    ...
    async sendMessage() {
      ...
      if (!this.attachments.length) {
        if (text.toLowerCase().indexOf('/tool ') === 0) {
          this.inputText = '';
          await this.executeUtcpToolCommand(text.slice(6));
          return;
        }
        if (text.toLowerCase().indexOf('!tool ') === 0) {
          this.inputText = '';
        ...
        if (text[0] === '!' && text.length > 1 && text[1] !== ' ') {
          this.inputText = '';
          await this.executeUtcpToolCommand(text.slice(1));
          return;
        }
      }
