...
function agentsPage() {
  return {
    ...
    /** Display name / description / category for builtin cards (i18n when available). */
    tplText(t, part) {
      var key = t._i18n && t._i18n[part === 'category' ? 'cat' : part];
      var fb =
        part === 'name'
          ? t.name
          : part === 'desc' || part === 'description'
            ? t.description
            : part === 'category' || part === 'cat'
              ? t.category
              : '';
      if (!key) return fb;
      try {
    ...
    /** Localize agent lifecycle state for badges (Running → 运行中, etc.). */
    ...
    async loadTemplates() {
      ...
      var builtinDefaults = [
        {
          _i18n: { name: 'wizard.general_assistant', desc: 'agents.builtin.general_desc', cat: 'wizard.cat_general' },
        ...
        {
          _i18n: { name: 'wizard.code_helper', desc: 'agents.builtin.code_desc', cat: 'wizard.cat_development' },
          name: 'Code Helper',
        ...
        {
          _i18n: { name: 'wizard.researcher', desc: 'agents.builtin.research_desc', cat: 'wizard.cat_research' },
          name: 'Researcher',
        ...
        {
          _i18n: { name: 'wizard.writer', desc: 'agents.builtin.writer_desc', cat: 'wizard.cat_writing' },
          name: 'Writer',
        ...
        {
          _i18n: { name: 'wizard.data_analyst', desc: 'agents.builtin.data_desc', cat: 'wizard.cat_development' },
          name: 'Data Analyst',
        ...
        {
          _i18n: { name: 'wizard.devops', desc: 'agents.builtin.devops_desc', cat: 'wizard.cat_development' },
      ...
      this.builtinTemplates = builtinDefaults.slice();
      try {
        ...
        var apiTemplates = (results[0].templates || [])
          .filter(function(t) {
            return t && typeof t.name === 'string' && t.name.trim().length > 0;
          })
          .map(function(t) {
            if (typeof t.description !== 'string') t.description = '';
            if (typeof t.category !== 'string' || !t.category.trim()) t.category = 'Custom';
            return t;
          });
        this.builtinTemplates = builtinDefaults.concat(apiTemplates);
