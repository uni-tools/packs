  ...
  return {
      ...
      OpenFangToast.confirm(
        ...
        async function() {
          try {
            ...
            OpenFangToast.success(t('sessions.toast_deleted'));
          } catch(e) {
            OpenFangToast.error(t('sessions.toast_delete_failed') + ': ' + e.message);
          }
        }
      );
    },
    ...
    async addKey() {
      ...
      try {
        ...
        this.showAdd = false;
        var t = window.i18n ? window.i18n.t.bind(window.i18n) : function(k) { return k; };
        OpenFangToast.success(t('sessions.toast_key_saved'));
        this.newKey = '';
        this.newValue = '""';
        await this.loadKv();
      } catch(e) {
        var t2 = window.i18n ? window.i18n.t.bind(window.i18n) : function(k) { return k; };
        OpenFangToast.error(t2('sessions.toast_key_save_failed') + ': ' + e.message);
      }
    },
      ...
      OpenFangToast.confirm(
        ...
        async function() {
          try {
            ...
            OpenFangToast.success(t('sessions.toast_key_deleted'));
            await self.loadKv();
          } catch(e) {
            OpenFangToast.error(t('sessions.toast_key_delete_failed') + ': ' + e.message);
          }
        }
      );
    },
    ...
    async saveEdit() {
      ...
      try {
        ...
        OpenFangToast.success(t('sessions.toast_key_updated'));
        this.editingKey = null;
        this.editingValue = '';
        await this.loadKv();
      } catch(e) {
        OpenFangToast.error(t('sessions.toast_save_failed') + ': ' + e.message);
      }
    }
  };
}
