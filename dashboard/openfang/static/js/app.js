...
// Register service worker from nonce-authorized bundled script context.
...
document.addEventListener('alpine:init', function() {
  function readUrlAuthToken() {
    try {
      var q = new URLSearchParams(window.location.search);
      var tok = q.get('token') || q.get('api_key');
      if (!tok && window.location.hash) {
        var h = window.location.hash.replace(/^#/, '');
        var qi = h.indexOf('?');
        if (qi >= 0) {
          q = new URLSearchParams(h.slice(qi + 1));
          tok = q.get('token') || q.get('api_key');
        }
      }
      return tok && tok.trim() ? tok.trim() : '';
    } catch (e) { return ''; }
  }

  // URL ?token= / ?api_key= (also works in hash query) for first-time browser setup
  var urlKey = readUrlAuthToken();
  if (urlKey) {
    localStorage.setItem('openfang-api-key', urlKey);
    OpenFangAPI.setAuthToken(urlKey);
  }
  ...
  Alpine.store('app', {
    ...
    async checkAuth() {
      ...
      // API key mode detection — use banner, not blocking modal (market/settings stay usable)
      try {
    ...
    submitApiKey(key) {
      ...
      this.showAuthPrompt = false;
      this.refreshAgents();
      window.dispatchEvent(new CustomEvent('openfang:auth-changed'));
    },
...
function app() {
  return {
    ...
    canShowPackPage: function(p) {
      if (p === 'skills') return this.packNav.loaded && this.packNav.skills;
      if (p === 'hands') return this.packNav.loaded && this.packNav.hands;
      if (p === 'utcp-tools') return this.packNav.loaded && this.packNav.utcp;
      if (p === 'channels') return this.packNav.loaded && this.packNav.channels;
      return true;
    },

    async refreshPackNav() {
      var self = this;
      var nav = { skills: false, hands: false, extensions: false, utcp: false, channels: false, loaded: true };
      try {
        var data = await OpenFangAPI.get('/api/market/installed');
        (data.items || []).forEach(function(item) {
          if (item.kind === 'skill') nav.skills = true;
          if (item.kind === 'hand') nav.hands = true;
          if (item.kind === 'extension') nav.extensions = true;
          if (item.kind === 'utcp-provider') nav.utcp = true;
        });
      } catch (_) {}
      try {
        var ch = await OpenFangAPI.get('/api/channels');
        var list = ch.channels || ch.items || ch || [];
        if (Array.isArray(list)) {
          nav.channels = list.some(function(c) {
            return c && (c.configured || c.enabled || c.status === 'connected' || c.status === 'running');
          });
        }
      } catch (_) {}
      self.packNav = nav;
      if (!self.canShowPackPage(self.page)) {
    ...
    init() {
      ...
      window.addEventListener('pack:changed', function() { self.refreshPackNav(); });
      window.addEventListener('openfang:auth-changed', function() { self.refreshPackNav(); });
      ...
      // Hash routing
      var validPages = ['overview','agents','sessions','approvals','comms','workflows','scheduler','channels','market','skills','hands','analytics','logs','runtime','settings','wizard','utcp-tools'];
      var pageRedirects = {
        'chat': 'agents',
        'templates': 'agents',
        'triggers': 'workflows',
      ...
      function handleHash() {
        ...
        if (self.packNav.loaded && !self.canShowPackPage(self.page)) {
          self.page = 'market';
    ...
    navigate(p) {
      if (this.packNav.loaded && !this.canShowPackPage(p)) {
        p = 'market';
      }
      this.page = p;
